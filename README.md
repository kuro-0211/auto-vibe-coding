...
③ Code Agent       — qwen2.5-coder 코드 생성 + 코드 리뷰
                       → ✏️ HITL: 사용자가 코드 직접 편집 가능 (Reflex 편집창)
                       → 승인 후 Docker 샌드박스 실행
...
사이드바 기반 6탭 구조의 대시보드를 제공합니다. Reflex 백그라운드 이벤트로 LangGraph 스트림을 받아 단계별 결과를 실시간 갱신합니다. 사이드바 하단에 **현재 시간 (KST)** 시계가 1초 단위로 자동 갱신되어 시연·스케줄 다음 트리거 가늠에 활용 가능합니다.
...
| 🕒 히스토리 | **수동 실행만** 자동 저장 + **PDF / Word / Markdown 재다운로드**, 성공/실패 뱃지, 전체 삭제 (스케줄러 자동 실행은 별도 페이지로 분리) |
| ⏰ 스케줄 | 매시간/매일/매주/매월 자동 실행 등록, 활성/비활성 토글, 다음 실행 시간 표시. 하단 **"📜 최근 스케줄 실행"** 섹션에서 자동 실행 결과 카드 → "결과 보기"로 상세(결과/리서치/코드/실행/에러) 이동 |
...
│   └── utils/
│       ├── logger.py
│       ├── history.py              ← 실행 히스토리 (수동/자동 schedule_id로 분리)
...
│       ├── components/
│       │   ├── layout.py
│       │   └── sidebar.py           · 📂 프로젝트 네비 + 현재 시간(KST) 시계
│       └── pages/
│           ├── run.py               · 실행 + HITL 코드 편집 + 프로젝트 선택 배너
│           ├── monitor.py
│           ├── log_page.py
│           ├── history_detail.py
│           ├── schedule_page.py     · 스케줄 등록/관리 + 📜 최근 스케줄 실행
